...
export type ScenarioProof = {
  ...
  pattern_evidence: {
    capture_environment_identity: null;
...
export async function generateScenarioProofMatrix(root: string): Promise<ScenarioProofIndex> {
  ...
  const captures = JSON.parse(await readFile(path.join(root, "artifacts/capture-results.json"), "utf8")) as { harnessHash: string; captures: CaptureRecord[] };
    ...
    for (const scenario of scenarioIds) {
      ...
      const proof: ScenarioProof = {
        ...
        closure: {
          dedicated_row: true,
          dedicated_artifact: true,
          pattern_specific_evidence: hasCaptureProof || hasRuntimeProof,
        ...
        gaps: [
          ...(!hasCaptureProof && !hasRuntimeProof ? ["current Patternに当該Scenarioの専用State/recordがない。"] : []),
          ...(hasCaptureProof && !hasRuntimeProof ? ["Capture recordはSource/Harness/Image digestを持つがBrowser versionを永続化していない。"] : []),
  ...
  const index: ScenarioProofIndex = {
    ...
    completion_limits: [
      ...
      "CaptureのBrowser version未記録をPerformance/Compatibilityの実Runtime identityで代替しない。",
