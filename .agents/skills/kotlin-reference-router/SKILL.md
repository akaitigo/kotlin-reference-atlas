...
description: Kotlinの設計、実装、診断、移行、レビューを、Kotlin 技術実証アトラスのCoverage、一次資料、再実行可能なLab、Evidenceへ案内する。Kotlin/JVM、Gradle、coroutines、Java相互運用の検証済み判断に使い、一般的な他言語質問やCoverage外機能の断定には使わない。
---
...
4. `disposition: gap`なら、未検証領域として明示し、存在しないCapabilityやEvidenceを捏造しない。必要なら`coverage.yaml`の`planned`または`partial` Targetを示す。
